window.log = function a() {
    ...
    if (this.console) {
        ...
        c = [].slice.call(b);
        if (typeof console.log === "object") log.apply.call(console.log, console, c); else console.log.apply(console, c);
    }
};
...
function styler(a) {
    return {
        get: function(b, c) {
            var c = c || false;
            if (c === true) {
                return parseInt(window.getComputedStyle(a, null).getPropertyValue(b), 10);
            } else {
...
if (typeof require !== "undefined" && typeof exports !== "undefined") {
    ...
    exports.main = function b(a) {
        if (!a[1]) throw new Error("Usage: " + a[0] + " FILE");
        var b, c;
        if (typeof process !== "undefined") {
            b = require("fs").readFileSync(require("path").resolve(a[1]), "utf8");
        } else {
            b = require("file").path(require("file").cwd()).join(a[1]).read({
                charset: "utf-8"
            });
        }
...
(function(a) {
    ...
    a.ui = {
        ...
        init: function() {
            var b = this;
            a.ui.navigationToggle();
            a.ui.navStickOnScroll();
        },
        navigationToggle: function() {
            a.ui.logo.addEventListener("click", function(b) {
                if (a.SiteSetup.navClosed === true) {
                    a.ui.openNav();
        ...
        openNav: function() {
            ...
            var b = a.ui.mastheadContent.scrollHeight;
            a.ui.mastheadContent.style[height] = b;
            a.ui.logo.classList.add("is-active");
            a.SiteSetup.navClosed = false;
        },
        closeNav: function() {
            a.ui.mastheadContent.style[height] = 0;
            a.ui.logo.classList.remove("is-active");
            a.SiteSetup.navClosed = true;
        },
        navStickOnScroll: function() {
            var b = a.ui.masthead.offsetTop;
            document.addEventListener("scroll", function(c) {
                var d = window.scrollY;
                if (!a.ui.navDocked && b - d < 0) {
                    a.ui.masthead.classList.add("stuck");
                    a.ui.navDocked = true;
                    console.log(a.ui.heroUnit.style);
                    a.ui.heroUnit.style[marginBottom] = a.ui.masthead.scrollHeight;
                } else if (a.ui.navDocked && b - d >= 0) {
                    a.ui.masthead.classList.remove("stuck");
                    a.ui.heroUnit.style[marginBottom] = 0;
                    a.ui.navDocked = false;
                }
                if (!a.SiteSetup.navClosed) {
                    a.ui.closeNav();
                    a.SiteSetup.navClosed = true;
                }
            });
        ...
        shotSwitch: function() {
            var a = /(shot-[0-9])/g, b = /(shot-)/g;
            $("#main").on(".layout-desktop-three-up figure", "click", function(a) {
                var b = $(this), c = b.siblings(".shot-visible").andSelf(), d = c.length - 1;
                if (b.hasClass("shot-1")) {
                    return false;
                } else if (b.hasClass("shot-2")) {
                    c.each(function(a) {
                        var b = parseInt($(this).data("index"), 10);
                        var c = "shot-" + a, e;
                        if (b >= d) {
                            b = 0;
                            e = "shot-" + b;
                        ...
                        $(this).removeClass().addClass(e).addClass("shot-visible").data("index", b);
                    });
                } else {
                    c.each(function(a) {
                        var b = parseInt($(this)[0].className.match(/shot-(\d+)/)[1], 10);
                        var c = "shot-" + a, d;
                        if (b === 0) {
                            b = 2;
                            d = "shot-" + b;
                        ...
                        $(this).removeClass().addClass(d).addClass("shot-visible");
                    });
                    console.log("prev");
                }
            });
            $("#main").on(".layout-desktop-two-up figure", "click", function(a) {
                if ($(this).hasClass("shot-1")) {
                    $(this).addClass("shot-0").removeClass("shot-1").siblings(".shot-0").removeClass("shot-0").addClass("shot-1");
                } else {
                    $(this).removeClass("shot-0").addClass("shot-1").siblings(".shot-1").removeClass("shot-1").addClass("shot-0");
                }
            });
        },
        getShotsHeight: function() {},
        shotHeight: function() {
            $("#main").on(".layout-stacked figure", "click", function(a) {
                var b = $(this).parent(), c = b[0].scrollHeight;
                console.log(b);
                console.log(c);
                if (b.hasClass("is-expanded")) {
                    b.removeClass("is-expanded").parent().css("paddingBottom", 0);
                } else {
                    b.addClass("is-expanded").parent().css("paddingBottom", "4%");
                }
            });
        }
    };

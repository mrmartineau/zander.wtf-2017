...
Please keep me up-to-date :)
	...
	3.) Smartresize - debounced window.resize
...
// Smart Resize - debounced window.resize
// Usage:
...
// USAGE:
// if ( isObjectDefined(TMW.PosterBoyPage) && checkForObject(TMW.PosterBoyPage) ) {
	//
// }
...
FacebookProxy = function (appID, permissions, channelURL, status, cookie, xfbml)
{
    this.APP_ID = appID;
    ...
    this.postToPhotosErrorSignal = new Signal ();
    this.postToPhotosSuccessSignal = new Signal ();
    this.checkPermissionsSignal = new Signal ();
    this.profilePicURLSignal = new Signal ();
    this.checkAuthStatusSignal = new Signal ();    

    this.initialised;
    this.userID = undefined;
    this.authenticated = false;

    // log("FacebookProxy:: [cconstructor] this.APP_ID = " + this.APP_ID);
};

FacebookProxy.constructor = FacebookProxy;

FacebookProxy.prototype.init = function ()
{
    // log("FacebookProxy:: [init]");

    var self = this;

    window.fbAsyncInit = function ()
    {
        // log("FacebookProxy:: [init - fbAsyncInit] this.APP_ID = " + self.APP_ID);
        ...
        log ("FacebookProxy:: [init] this.checkAuthStatusSignal " + this.checkAuthStatusSignal);
    };
...
FacebookProxy.prototype.login = function (useRedirect, redirectURI)
{
    var self = this;
    
    if (redirectURI && redirectURI != "") self.redirectURI = redirectURI;
    ...
    if (useRedirect)
    {
        ...
        oauth_url += "&redirect_uri=" + encodeURIComponent (self.redirectURI);
    ...
    else
    {
        ...
        function loginResponseHandler (response)
        {
            ...
            {
                // log("FacebookProxy:: user has auth'd app and is logged into Facebook... dispatching 'facebookAuthenticatedSignal'");
            ...
            {
                // log("FacebookProxy:: [login] user cancelled login.");
...
FacebookProxy.prototype.checkPermissions = function (permissionsArray)
{
    var self = this;
    var match = false;
    ...
    {
        // log ("FacebookProxy:: [checkPermissions] response:\n\n"  + response + "\n\n");
            ...
            if (responseDataLength > 0)
            {
                for (i; i < length; ++i)
                {
                    ...
                    for (j; j < responseDataLength; ++j)
                    {
                        ...
                        {
                            // log("FacebookProxy:: [checkPermissions] found permission: " + currentPermissionString);
                            match = true;
                            break;
                        }
                    }
...
FacebookProxy.prototype.checkAuthStatus = function ()
{
    var self = this;
    ...
    function getLoginStatusHandler (response)
    {
        if (response.status === "connected")
        {
            ...
            log ("FacebookProxy:: [checkAuthStatus] userID = " + self.userID);
            
            self.checkAuthStatusSignal.dispatch (true);
        }
        else
        {
            self.checkAuthStatusSignal.dispatch (false);
        }
    }
}

FacebookProxy.prototype.postToFeed = function (postObject, useAPI)
{
    // log("FacebookProxy:: [postToFeed]");
    ...
    function feedPostResponseHandler (response)
    {
        ...
        {
                // log("FacebookProxy:: [feedPostResponseHandler] Error posting to user's Facebook feed");
                
                self.postToFeedErrorSignal.dispatch ();
        }
        else
        {
                // log("FacebookProxy:: [feedPostResponseHandler] successfully posted to user's Facebook feed");
...
FacebookProxy.prototype.postToPhotos = function (imageURL, messageText)
{
    var self = this;
    
    FB.api ("/me/photos", 'post', {url: imageURL, message: messageText}, photosPostResponseHandler);
    
    // log ("FacebookProxy:: [postToPhotos] messageText = " + messageText);
    
    function photosPostResponseHandler (response)
    {
        if (!response || response.error)
        {
                // log("FacebookProxy:: [photosPostResponseHandler] Error posting to user's Facebook photos");

                self.postToPhotosErrorSignal.dispatch (response);
        }
        else
        {
                // log("FacebookProxy:: [photosPostResponseHandler] successfully posted to user's Facebook photos");
                log ("FacebookProxy:: [photosPostResponseHandler] response: ");
                log (response);
                
                self.postToPhotosSuccessSignal.dispatch (response.id, response.post_id);
        }
    }
}

FacebookProxy.prototype.createTagObject = function (userIDString, posX, posY)
{
    var tagObject = {to: userIDString, x: posX, y: posY};
    
    return tagObject;
};


FacebookProxy.prototype.tagPhoto = function (photoID, tagObjectsArray)
{
    var self = this;
    
    // FB.api ("/me/photos/" + photoID + "tags", 'post', {tags: tagObjectArray}, photosPostResponseHandler);
    FB.api ("/" + photoID + "/tags", 'post', {tags: tagObjectArray}, photosPostResponseHandler);
    ...
    function photosPostResponseHandler (response)
    {
        ...
        else
        {
                ...
                self.postToPhotosSuccessSignal.dispatch ();
        }
    }
}

FacebookProxy.prototype.setIframeSize = function (canvasWidth, canvasHeight)
{
    // FB.Canvas.setAutoGrow (false);
    FB.Canvas.setSize ({width: canvasWidth, height: canvasHeight});
};

FacebookProxy.prototype.getProfilePicURL = function (width)
{
    var self = this;
    var profilePicURL;

    function profilePicURLHandler (response)
    {
        profilePicURL = response.data.url;
        self.profilePicURLSignal.dispatch (profilePicURL);
    }

    FB.api ("/me/picture?width=" + width, profilePicURLHandler);
};


/* **********************************************
     Begin director.min.js
********************************************** */
